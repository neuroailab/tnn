...
def custom_harbor(inputs, shape, spatial_op='resize', channel_op='concat', kernel_init='xavier', reuse=None):
    """
        ...
        if len(shape) == 2:
            ...
            if len(inp.shape) == 2:
                ...
                if spatial_op == 'tile':
                    ...
                    with tf.variable_scope(nm, reuse=reuse):
                        box_init = tf.zeros_initializer()
                        boxes = tf.get_variable(initializer=box_init,
                                                shape=[shape[0], 4],
                                                dtype=tf.float32,
                                                name='boxes')
                        boxes = tf.nn.sigmoid(boxes) # keep values in [0, 1] range
                        out = tf.image.crop_and_resize(inp, boxes, box_ind, tf.constant([shape[1], shape[2]]))
                else:
                    out = tf.image.resize_images(inp, shape[1:3])
